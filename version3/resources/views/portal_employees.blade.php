...
<div id="settingsBox">
    ...
    <table id="empTable">
        ...
        @foreach ($employees as $employee)
            <tr>
                ...
                <td><a href="/portal-emp-timesheets/{{$employee['id']}}"><span class="empTime">View Timesheet</span></a> | <a href="/portal-edit-employee/{{$employee['id']}}"><span class="editEmp">Edit</span></a> | <a href="/portal-delete-employee/{{$employee['id']}}"><span class="delEmp">Delete</span></a></td>
            </tr>
        @endforeach
        
    </table>
    
    @if ( count($delEmployees) > 0 )
    ...
    <table id="empTable">
        ...
        @foreach ($delEmployees as $employee)
            <tr>
                <td>{{$employee['id']}}</td>
                <td class="empsName">{{$employee['name']}}</td>
                <td>{{$employee['type']}}</td>
                <td>{{$employee['username']}}</td>
                <td>{{$employee['location']}}</td>
                <td><a href="/portal-restore-employee/{{$employee['id']}}"><span class="delEmp">Restore Employee</span></a></td>
            </tr>
        @endforeach
